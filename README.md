...
| `bundesland-widget-snippet-heat-pump.js` | Loader that injects the heat pump iframe + forwards funnel params |
...
| `publisher-content` | `welt-waermepumpe-article` | Free-form label (Zapier → Tableau) |
...
```
https://www.top10-anbieter.de/waermepumpe-desktop-2?utm_source=welt.de&utm_medium=advertorial&utm_campaign=wp_june26&bcid=3jf95jdleq&publisher=Welt&publisher-content=welt-waermepumpe-article&utm_content=BY&bundesland=BY
```
...
Both widgets also try to forward `gclid`, `msclkid`, `matchtype`, `utm_source`,
`utm_campaign`, `keyword`, `placement`, and `device` from the page URL / referrer
to the tile links, for Google/Microsoft Ads attribution. This is currently
**dormant**: Welt's referrer policy strips the params and Welt IT declined to add a
forwarding script, so nothing arrives. See `CONTEXT.md` for the full story.
