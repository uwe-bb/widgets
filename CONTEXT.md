...
All three widgets forward `gclid`, `wbraid`, `gbraid`, `msclkid`, `utm_term`, `matchtype`, `keyword`, `placement`, `device` from the widget's own src URL (and `document.referrer` as a fallback) to the tile links (`wbraid`/`gbraid` added in commit `25410eb`; `utm_term` added 2026-09-22 as the interim carrier for the Google campaign ID via final URL suffix `utm_term={campaignid}` — Welt's script already forwards `utm_term` and no article link hardcodes it; the DWH reads it as the Ads campaign ID for Welt bcids until `utm_campaign` stops being hardcoded on the pages. Spec: https://claude.ai/code/artifact/5132ff2a-4be9-4924-99a6-e2cb2464d61c). `utm_source` / `utm_campaign` are deliberately NOT passed through — they stay hardcoded to `welt.de` / the month tag so traffic always attributes to Welt.
